...
//       pool.query(
//         `SELECT COUNT(*) AS total FROM tbl_employee WHERE status = 'Active'`,
//       ),
...
exports.getDashboardData = async (req, res) => {
  try {
    const [
      ...
      // Absent today - ONLY from status = Absent
      pool.query(`
      ...
      pool.query(`
        SELECT
          ...
          COUNT(DISTINCT employee_id)
            FILTER (WHERE status IN ('Present', 'Late')) AS present,

          COUNT(DISTINCT employee_id)
            FILTER (WHERE status = 'Late') AS late,

          COUNT(DISTINCT employee_id)
            FILTER (WHERE status = 'Absent') AS absent
    ...
    const present = parseInt(presentResult.rows[0].present, 10);
    const late = parseInt(lateResult.rows[0].late, 10);
    const absent = parseInt(absentResult.rows[0].absent, 10);
    ...
    const punchActivity = punchActivityResult.rows.map((row) => {
      ...
      return {
        ...
        punchInTime: new Date(row.punch_in).toLocaleTimeString(
          "en-US",
          {
            hour: "2-digit",
            minute: "2-digit",
